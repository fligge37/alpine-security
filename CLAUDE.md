...
`apps/web` hat einen ersten Ende-zu-Ende-Flow für Wanderer (Login per Handynummer/OTP, Tour starten, Standort-Ping manuell senden, Tour beenden, Link für Angehörige erzeugen/widerrufen) unter `apps/web/src/screens`, angebunden über `apps/web/src/api/client.ts` – inklusive einer öffentlichen, loginfreien Ansicht unter `/share/:token` (`ShareScreen.tsx`, Weiche dafür pfadbasiert in `App.tsx`, kein Router nötig für diese eine Route) mit eigener MapLibre-Kartenansicht (`apps/web/src/components/ShareMap.tsx`, Track als Linie statt nur letztem Marker, sonst gleiches Setup wie im Dashboard inkl. der `optimizeDeps`-Falle unten). `apps/dashboard` hat den entsprechenden Flow für die Bergwacht (Login per E-Mail/Passwort, Touren-Suche nach Status `aktiv`/`beendet` inkl. letztem bekannten Standort) unter `apps/dashboard/src/screens`, angebunden über `apps/dashboard/src/api/client.ts` – inklusive einer echten 3D-Kartenansicht (`apps/dashboard/src/components/ToursMap.tsx`) mit Markern für den letzten bekannten Standort pro Tour: MapLibre GL JS (Open Source) mit OpenFreeMap als Basiskarte (`https://tiles.openfreemap.org/styles/liberty`) und AWS-Open-Data-Terrain-Tiles (`elevation-tiles-prod`, Terrarium-Encoding) für die Höhendaten – beides kostenlos, kein API-Key. **Wichtige Vite-Falle:** `maplibre-gl` braucht `optimizeDeps: { exclude: ['maplibre-gl'] }` in `vite.config.ts`, sonst wird der Web-Worker nicht mitausgeliefert (404) und die Karte rendert nur den Hintergrund ohne Straßen/Labels/Terrain – kein Fehler in der Konsole, nur ein `net::ERR_FAILED` für `maplibre-gl-worker.mjs`. Beide Apps: im Dev-Betrieb läuft je ein Vite-Proxy (`/api` → `localhost:3000`) statt CORS im Backend. `apps/api` hat ein migriertes Datenmodell (`region`, `app_user`, `rescue_org_member`, `tour`, `location_ping`, `otp_code` – siehe ADRs 0001–0006, 0008) und erste Endpoints:
...
- Teilbarer Tour-Link für Angehörige (ADR 0006): `POST /tours/:id/share` (Link erzeugen/rotieren, nur Tour-Owner), `DELETE /tours/:id/share` (widerrufen), `GET /share/:token` (öffentlich, kein Login – Linkbesitz ist der Autorisierungsnachweis, liefert kompletten Tourverlauf); Link ist gültig während `aktiv` plus 24h nach Tourende, danach 404 unabhängig vom Link selbst
- Löschfristen für den Standortverlauf (ADR 0008): `location_ping` wird 7 Tage nach `tour.ended_at` automatisch gelöscht (Operator-Script `apps/api/src/scripts/delete-expired-pings.ts`, noch nicht per Cron eingeplant – siehe „Offene Punkte"); die Bergwacht kann einzelne Touren im Ernstfall über `POST /rescue/tours/:id/hold` / `DELETE /rescue/tours/:id/hold` bis auf Widerruf davon ausnehmen (UI-Button dafür in `apps/dashboard`s `ToursScreen`)
...
- **Datensparsamkeit/DSGVO beachten.** Standortverlauf nach Tourende zeitnah löschen; gestaffelte Löschfristen für den Ernstfall bewusst im Datenmodell abbilden, nicht als nachträglicher Fix. Umgesetzt in [ADR 0008](docs/adr/0008-loeschfristen-standortverlauf.md): 7-Tage-Basisfrist plus manueller Hold durch die Bergwacht für tatsächliche Ernstfälle.
